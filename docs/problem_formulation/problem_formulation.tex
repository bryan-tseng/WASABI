...
\begin{equation}
    \mathcal{L}(r; \lambda) = \prod_{t=1}^T \prod_{j=1}^M \frac{e^{-\lambda_j} (\lambda_j)^{r_j}}{r_j!}
\end{equation}
...
\[
\begin{aligned}
    \mathbb{E}[\lambda_t\mid \textbf{x}_{t}]=\hat{\textbf{D}}_{t-1}\hat{\textbf{W}}_{t-1}\textbf{x}_{t}+\textbf{b}^{prestim}
\end{aligned}
\]
where $\hat{\textbf{D}}_{t-1}$ and $\hat{\textbf{W}}_{t-1}$ are estimated from timepoints $i = 1,\ldots, t-1$
and $\textbf{b}^{prestim}$ is baseline firing rate identified from the prestim period.
