...
\mathbf{y}_t &\sim \text{Poisson}\!\left(\text{softplus}\!\left(\mathbf{C}\mathbf{x}_t + \mathbf{d} + \mathbf{D}\mathbf{s}_t\right)\right)\\
\boldsymbol{\theta} &= \{\mathbf{A},\, \mathbf{C},\, \mathbf{d},\, \boldsymbol{\mu}_0,\, \boldsymbol{\Sigma}_0\}
\end{align*}
...
\end{align}
which we maximize directly via SGD.

\paragraph{Emission Jacobian}
Let $\boldsymbol{\lambda}_t = \text{softplus}(\mathbf{C}\hat{\mathbf{x}}_t + \mathbf{d})$.
The Jacobian of the log-rate with respect to the latent state is:
\begin{align}
...
\end{align}
where $\sigma(\cdot)$ is the sigmoid (derivative of softplus).

\paragraph{EKF forward pass (predict--update)}
For $t = 1, \dots, T$:
\begin{align}
...
The Gaussian approximation yields the tractable objective:
\begin{align}
    \mathcal{L}(\boldsymbol{\theta})
        &\approx \sum_{t=1}^{T} \log \mathcal{N}(\mathbf{y}_t;\,
                 \boldsymbol{\lambda}_{t|t-1},\, \mathbf{S}_t)
...
\end{align}
where $\eta_\tau$ is the learning rate at step $\tau$ and gradients are
computed via automatic differentiation (JAX) through the unrolled filter.
